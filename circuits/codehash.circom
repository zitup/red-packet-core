...
template CodeHasher() {
    signal input packetAddress;  // 红包地址
    ...
    // finalHasher.inputs[2] <== packetAddress;    // 红包地址

    // out <== finalHasher.out; // 最终哈希输出

    // 将三个值拼接在一起
    // passwordHash (32 bytes) + packetAddress (20 bytes) + userAddress (20 bytes)
    // out <== poseidon.out * (1 << 320) + // 先移位 (20+20)*8 位
    //             packetAddress * (1 << 160) + // 再移位 20*8 位
    //             userAddress; // 最后放用户地址
}
